...
from app.scraper import fetch_and_parse, scrape_links_to_database, discover_links
...
class TestTrafilaturaIntegration:
    ...
    def test_fetch_and_parse_plain_text_fallback(self):
        ...
        with patch('trafilatura.fetch_url') as mock_trafilatura_fetch, \
             patch('app.scraper.Article') as mock_article_class, \
            ...
            mock_response.text = "This is plain text content that should be extracted directly from the response."
            ...
            assert content == "This is plain text content that should be extracted directly from the response."
        ...
        try:
            with patch('app.scraper.fetch_and_parse') as mock_fetch:
                mock_fetch.side_effect = [
                    ("Test Article 1", "This is the content of test article 1 with sufficient length to pass validation."),
                    ("Test Article 2", "This is the content of test article 2 with sufficient length to pass validation.")
                ]
